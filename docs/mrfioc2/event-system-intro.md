...
There are 256 event codes available, most of them freely configurable by
...
to be taken. For example, in the context of an accelerator, an event
...
with a quarter of the event clock rate time resolution (40 ns at 100 MHz
...
the highest time resolution for the distributed bus bits was half of the event clock rate.
```
